Shader "Chaos/Diff-Norm-Spec-Lighting"
{
	Properties{
...
/*
				...
				half3 h = normalize(_CHAOSLightDirection + viewDirection);
				float nh = max(0, dot(normalDirection, h));
				float spec = saturate(pow(nh, (48.0 * specTex.a)));
				//float spec = pow(nh, 48.0);
				specTex *= (spec * _CHAOSLightMultiplier);
*/
#ifdef CAUSTICS
			half2 yUV = i.posWorld.xz / _TextureScale;
			half2 xUV = i.posWorld.zy / _TextureScale;
			half2 zUV = i.posWorld.xy / _TextureScale;

			half3 yDiff = tex2D (_CausticTex, yUV);
			half3 xDiff = tex2D (_CausticTex, xUV);
			half3 zDiff = tex2D (_CausticTex, zUV);

			half3 blendWeights = pow (abs(normalDirection), _TriplanarBlendSharpness);

			blendWeights = blendWeights / (blendWeights.x + blendWeights.y + blendWeights.z);
			half3 finalCaustics = xDiff * blendWeights.x + yDiff * blendWeights.y + zDiff * blendWeights.z;

			finalCaustics *= _CausticColor.rgb;

			//Dont Render Caustics on bottom
			half scale = saturate(dot(i.wNormal, half3(0, 1, 0)) + _CausticFalloff);

			diff += (finalCaustics * _CausticAmount * scale);
